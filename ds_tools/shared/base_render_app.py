...
class BaseRenderApp(ShowBase):
    ...
    def __init__(self, title='Panda3d App', width=None, height=None, headless=False):
        ...
        for obj in self.camera.getChildren():
            self.main_camera_np = obj
        self.main_camera = self.main_camera_np.node()
    ...
    def capture_screenshot(self, source=None):
        ...
        if self.headless:
            attempt_source = self.headless_buffer
            self.screenshot(temp_screenshot_file, source=attempt_source, defaultFilename=False)
        else:
